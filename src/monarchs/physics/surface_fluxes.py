...
def sfc_flux(
    ...
    T_rock : float
        Surface temperautre of exposed rocks [K].
    wind : float
        Wind speed. [m s^-1].
    xsurf : float
...
def bulk_fluxes(wind, T_air, T_sfc, p_air, T_dp):
    ...
    # testing a different way to calculate Ri to avoid ValueErrors
    Ri = np.divide(
        g * (T_air - T_sfc) * dz,
        T_air * wind**2,
    ...
    # testing a different way to calculate CT to avoid ValueErrors
    CT_unstable = CT0 * (1 - 2 * b * Ri / (1 + c * np.abs(Ri) ** 0.5))
    CT_stable   = CT0 * (1 + b * Ri) **-2
    CT = np.where(Ri < 0, CT_unstable, CT_stable)
